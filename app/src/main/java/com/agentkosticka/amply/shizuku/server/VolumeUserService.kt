...
open class VolumeUserService : IVolumeService.Stub() {
    ...
    private val fractionalVolumeBackend = FractionalStreamVolumeBackend(
        snapshotReader = ::readStreamVolumeSnapshot
    )
    ...
    private fun clearAudioService() {
        ...
        fractionalVolumeBackend.invalidateAll()
    }

    private fun readStreamVolumeSnapshot(streamType: Int): StreamVolumeSnapshot? {
        ...
        return runCatching {
            val min = (runCatching {
                invokeAudioService(service, "getStreamMinVolume", streamType) as? Number
            }.getOrNull()?.toInt() ?: 0).coerceAtLeast(0)
            val max = (invokeAudioService(service, "getStreamMaxVolume", streamType) as? Number)
                ?.toInt() ?: return@runCatching null
            val current = (invokeAudioService(service, "getStreamVolume", streamType) as? Number)
                ?.toInt() ?: return@runCatching null
            StreamVolumeSnapshot(min, max, current).takeIf { it.valid }
        }.getOrNull()
    }
    ...
    override fun getCapabilities(): Long = VOLUME_PROTOCOL_REQUIRED_CAPABILITIES or
        if (fractionalVolumeBackend.isAvailable(AudioManager.STREAM_MUSIC)) {
            CAPABILITY_FLOAT_STREAM_VOLUME
        } else {
            0L
        }
    ...
    override fun setSystemStreamVolume(streamType: Int, index: Int): OperationResultParcel {
        fractionalVolumeBackend.invalidate(streamType)
    ...
    override fun setSystemStreamVolumeFloat(streamType: Int, gain: Float): OperationResultParcel {
        ...
        return when (fractionalVolumeBackend.apply(streamType, gain)) {
            is FractionalApplyResult.Applied -> OperationResultParcel.success(verified = false)
            FractionalApplyResult.Unsupported ->
                OperationResultParcel.failure(STATUS_UNSUPPORTED, "fractional_stream_unsupported")
            FractionalApplyResult.Failed ->
                OperationResultParcel.failure(STATUS_FAILED, "fractional_stream_failed")
        }
    }

    @Synchronized
    override fun getSystemStreamVolumeFloatState(streamType: Int): FractionalVolumeStateParcel {
        return fractionalVolumeBackend.state(streamType)
    }

    @Synchronized
    override fun invalidateSystemStreamVolumeFloat(streamType: Int) {
        fractionalVolumeBackend.invalidate(streamType)
    }
