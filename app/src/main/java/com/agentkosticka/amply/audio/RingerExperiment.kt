...
class RingerExperimentExecutor(
    ...
    suspend fun testAllTransitions(method: RingerExperimentMethod) {
        ...
        _methodTestProgress.value = _methodTestProgress.value + (method to 0f)
        ...
        try {
            ...
            ALL_RINGER_MODE_TRANSITIONS.forEachIndexed { index, (from, to) ->
                ...
                _methodTestProgress.value = _methodTestProgress.value +
                    (method to ((index + 1f) / ALL_RINGER_MODE_TRANSITIONS.size))
            }
        } catch (e: Exception) {
            overallDetail = e.message ?: e.javaClass.simpleName
        } finally {
            _methodTestResults.value = _methodTestResults.value +
                (method to RingerMethodTestResult(method, results, overallDetail))
            _methodTestProgress.value = _methodTestProgress.value + (method to 1f)
    ...
    suspend fun execute(
        ...
        _results.value = _results.value + (method to result)
        _busy.value = false
        return result
    }
